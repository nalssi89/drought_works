...
import { FUTURE_PERIOD } from "../lib/future-scenario";
...
export function Controls({ date, period, intraday, observationTime, maximumObservationTime, liveLatest }: ControlsProps) {
  ...
  const nowHref = intraday ? `/?period=${period}&intraday=1` : `/?period=${period}`;
  ...
  return (
    <section className="control-panel" aria-label="조회 조건">
      ...
      <nav className="periods" aria-label="누적기간 및 시나리오">
        <span>누적기간</span>
        {PERIOD_OPTIONS.map(([value, label]) => {
          const href = value === FUTURE_PERIOD
            ? `/?period=${FUTURE_PERIOD}${intraday ? "&base=intraday" : ""}`
            : liveLatest
              ? `/?period=${value}${intraday ? "&intraday=1" : ""}`
              : query(date, value, intraday, intraday ? observationTime : null);
          return <Link className={value === period ? "period-button selected" : "period-button"} href={href} key={value}>{label}</Link>;
        })}
      </nav>
